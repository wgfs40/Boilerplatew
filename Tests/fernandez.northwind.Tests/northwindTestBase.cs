...
namespace fernandez.northwind.Tests
{
    public abstract class northwindTestBase : AbpIntegratedTestBase<northwindTestModule>
    {
        ...
        protected void UsingDbContext(Action<northwindDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<northwindDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<northwindDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<northwindDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<northwindDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<northwindDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<northwindDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<northwindDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<northwindDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<northwindDbContext, Task<T>> func)
        {
            T result;
